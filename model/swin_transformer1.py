...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, grayscale):
        ...
        self.layer5 = self._make_layer(block, 768, layers[3], stride=2)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.mlp_ratio = mlp_ratio
        self.resnet = ResNet(block=BasicBlock, layers=[3, 4, 6, 3], num_classes=num_classes, grayscale=False)
        ...
        self.head = nn.Linear(160, 1, bias = True)
        ...
        self.feature = nn.Sequential(
            ...
            nn.Linear(in_features=self.num_features, out_features=1, bias=False),
            nn.BatchNorm1d(num_features=1, eps=2e-5)
        )
    ...
    def forward(self, x):
        ...
        #x = x + global_features + local_features
        x = x + local_features
        x = self.feature(x)
        y = x.squeeze()
        logits = x + self.linear_1_bias
        #print(y.shape)
        probas = torch.sigmoid(logits)
        #probas = torch.distributions.Normal(0, 1).cdf(y)
