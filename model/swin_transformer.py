...
class FocalTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.fc = nn.Linear(self.num_classes-1, 1, bias=False)
        self.linear_1_bias = nn.Parameter(torch.zeros(self.num_classes-1).float())
        self.linear_2_bias = nn.Parameter(torch.zeros(self.num_classes-1).float())
        #self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
        
        self.feature = nn.Sequential(
            nn.Linear(in_features=self.num_features, out_features=self.num_features // 2, bias=False),
            nn.BatchNorm1d(num_features=self.num_features // 2, eps=2e-5),
            nn.Linear(in_features=self.num_features // 2, out_features=self.num_classes-1, bias=False),
            nn.BatchNorm1d(num_features=self.num_classes-1, eps=2e-5)
        )
        self.feature1 = nn.Sequential(
            nn.Linear(in_features=65856, out_features=self.num_features, bias=False),
            nn.BatchNorm1d(num_features=self.num_features, eps=2e-5),
            nn.Linear(in_features=self.num_features, out_features=768, bias=False),
            nn.BatchNorm1d(num_features=768, eps=2e-5)
        )
        self.feature2 = nn.Sequential(
            nn.Linear(in_features=37632, out_features=self.num_features, bias=False),
            nn.BatchNorm1d(num_features=self.num_features, eps=2e-5),
            nn.Linear(in_features=self.num_features, out_features=768, bias=False),
            nn.BatchNorm1d(num_features=768, eps=2e-5)
        )
    ...
    def forward_features(self, x):
        ...
        B, L, C, W = local_features.shape
        local_features = local_features.reshape(B, L* C * W)
        # B, C, H, W
        global_features = x
        ...
        global_features = global_features.reshape(B, C * H * W)
        # B, C, H, W
    ...
    def forward(self, x):
        ...
        local_features = self.feature1(local_features)
        global_features = self.feature2(global_features)
        
        x = self.feature(x)
        probas = torch.sigmoid(x)
        local_features = self.feature(local_features)
        global_features = self.feature(global_features)
        local_features = self.fc(local_features)
        global_features = self.fc(global_features)
        y = local_features + global_features
        y = torch.sigmoid(y)
        y = y.squeeze()
        ...
        #local_features = local_features + self.linear_1_bias
        #local_features = torch.sigmoid(local_features)
        
        #global_features = global_features + self.linear_2_bias
        #global_features = global_features.squeeze()
        #global_features = torch.sigmoid(global_features)
        #x = self.fc(x)
        #local_features = local_features.squeeze()
        #logits = x + local_features + global_features
        #y = logits.squeeze()
        #global_features = global_features.squeeze()
        #z = y + global_features
        #z = torch.sigmoid(z)
