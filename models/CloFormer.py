...
class CloFormer(nn.Module):
    ...
    def __init__(self, in_chans, num_classes, embed_dims: List[int], depths: List[int],
        ...
        self.feature = nn.Sequential(
            nn.Linear(in_features=embed_dims[-1], out_features=embed_dims[-1]*2**(self.num_layers-1), bias=False),
            nn.BatchNorm1d(num_features=embed_dims[-1]*2**(self.num_layers-1), eps=2e-5),
            nn.Linear(in_features=embed_dims[-1]*2**(self.num_layers-1), out_features=num_classes, bias=False),
