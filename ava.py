...
TRAIN_CSV_PATH = './dataset/ava1_train.csv'
TEST_CSV_PATH = './dataset/ava1_test.csv'
IMAGE_PATH = './images_crop'
...
class CACDDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        avgscore = self.y[index]
        levels = [1]*label + [0]*(NUM_CLASSES - 1 - label)
        levels = torch.tensor(levels, dtype=torch.float32)
        label = self.label[index]

        return img, avgscore, levels
...
def compute_mae_and_mse(model, data_loader, device):
    ...
    for i, (features, targets, levels) in enumerate(data_loader):
        ...
        mae += torch.sum(torch.abs(preb - targets))
        mse += torch.sum((preb - targets)**2)
    mae = mae.float() / num_examples
    mse = mse.float() / num_examples
    preb = preb.cpu().numpy()
    x = targets.cpu().numpy()
    pc = torch.sum(torch.tensor(pearsonr(preb, x)[0]))
    return mae, mse, pc
...
for epoch in range(num_epochs):
    ...
    for batch_idx, (features, targets, levels) in enumerate(train_loader):
        ...
        predict_levels = probas > 0.5
        predicted_labels = torch.sum(predict_levels, dim=1)
        probas = predicted_labels + score
        cost2 = loss(probas.float(), targets.float())
    ...
    s = 'MAE/RMSE/PLCC: | Current test: %.4f/%.4f/%.4f Ep. %d | Best test : %.4f/%.4f/%.4f Ep. %d' % (
...
with torch.set_grad_enabled(False):  # save memory during inference
    ...
    s = 'MAE/RMSE: | Train: %.4f/%.4f/%.4f | Test: %.4f/%.4f/%.4f' % (
...
with torch.set_grad_enabled(False):
    ...
    s = 'MAE/RMSE/PC: | Best Train: %.4f/%.4f/%.4f | Best Test: %.4f/%.4f/%.4f' % (
        train_mae, torch.sqrt(train_mse), train_pc,
        test_mae, torch.sqrt(test_mse), test_pc)
    print(s)
    with open(LOGFILE, 'a') as f:
